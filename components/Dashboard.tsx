...
export const Dashboard: React.FC<DashboardProps> = ({ houses, onNavigate, onSelectHouse }) => {
  ...
  const totalRequiredDocs = totalActiveHouses * REQUIRED_DOCS.length;
  const totalSubmittedDocs = activeHouses.reduce((acc, h) => acc + h.documents.filter(d => d.isSubmitted).length, 0);
  ...
  const stageProgress = stagesConfig.map(stage => {
    ...
    const totalDocsInStage = totalStageHouses * REQUIRED_DOCS.length;
    const submittedDocsInStage = stageHouses.reduce((acc, h) => 
        acc + h.documents.filter(d => d.isSubmitted).length, 0
    );
  ...
  const docsByStage = useMemo(() => {
    ...
    const globalDocBreakdown = REQUIRED_DOCS.map(docDef => {
        const submittedCount = activeHouses.filter(h => 
            h.documents.find(d => d.id === docDef.id)?.isSubmitted
        ).length;
    ...
    const stageStats = stagesConfig.map(stage => {
        ...
        const docBreakdown = REQUIRED_DOCS.map(docDef => {
